    ...
    <div class="panel">
        ...
        <div class="panel-body">
                        ...
                        <select name="category" class="form-control team-select" required>
                            ...
                            @if($category)
                                @foreach($category as $row)
                                    <option value="{{$row->id}}" {{ ($row->id == $data->category) ? 'selected' : '' }}>
                                        {{$row->category}}</option>
                                @endforeach
                            @endif
                        </select>
            ...
            <div class="form-group">
                ...
                <div class="col-lg-9">
                    <div class="bs-component">
                        <textarea class="form-control textarea" name="highlights"
                            rows="6">{{$data->highlights}}</textarea>
                    </div>
                </div>
            </div>
  ...
  <div class="sid_bvijay mb10">
   <label class="field text">
     <input type="text" id="" name="linkedin_url" class="form-control" placeholder="Linkedin Link" value="{{$data->linkedin_url}}" />
        ...
        {{-- <div class="sid_bvijay mb10">
            <h4> Banner </h4>
            <h4> <input type="checkbox" name="published" value="{{ $data->published }}" {{ ($data->published ==
                1)?'checked':'' }} /> Blur </h4>
            <div class="hd_show_con">
                <div id="xedit-demo">
                    @if($data->banner)
